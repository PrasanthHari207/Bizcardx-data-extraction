...
st.set_page_config(page_title="Bizcardx data extraction by Prasanth !", layout="wide")
...
if selected == 'Home':
    ...
    if selected_mode == 'View':
        ...
        with col5:
            query = "SELECT name, designation, company, email, website, primary_no, secondary_no, address, pincode FROM business_cards WHERE name = %s"
        cursor.execute(query, (selected_contact,))
        y = cursor.fetchall()
        st.table(pd.DataFrame(y, columns=['Name', 'Designation', 'Company', 'Email ID', 'Website', 'Primary Contact', 'Secondary Contact', 'Address', 'Pincode']))
